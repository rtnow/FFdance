...
class LookCloserAgent:
    ...
    def update_critic(self, aug_obs, action, reward, discount, aug_next_obs, step):
        metrics = dict()
    ...
    def update(self, replay_iter, step, traj):
        ...
        original_fix_next_obs = aug_fix_next_obs.clone()
        original_ego_next_obs = aug_ego_next_obs.clone()
        if l % 3 == 0:
            # strong_aug_fix_obs = random_color_jitter(original_fix_obs)
            # strong_aug_ego_obs = random_color_jitter(original_ego_obs)
            #
            # strong_aug_fix_next_obs = random_color_jitter(original_fix_next_obs)
            # strong_aug_ego_next_obs = random_color_jitter(original_ego_next_obs)
            ...
            strong_aug_fix_next_obs = random_mask_freq_v2(random_overlay(original_fix_next_obs))
            strong_aug_ego_next_obs = random_mask_freq_v2(random_overlay(original_ego_next_obs))
        else:
            # strong_aug_fix_obs = random_color_jitter(original_fix_obs[:, :l-1])
            # strong_aug_ego_obs = random_color_jitter(original_ego_obs[:, :l-1])
            ...
            # strong_aug_fix_next_obs = random_color_jitter(original_fix_next_obs[:, :l - 1])
            # strong_aug_ego_next_obs = random_color_jitter(original_ego_next_obs[:, :l - 1])
            strong_aug_fix_next_obs = random_mask_freq_v2(random_overlay(original_fix_next_obs[:, :l - 1]))
            strong_aug_ego_next_obs = random_mask_freq_v2(random_overlay(original_ego_next_obs[:, :l - 1]))
            strong_aug_fix_next_obs = torch.cat([strong_aug_fix_next_obs, original_fix_next_obs[:, l - 1:l]], dim=1)
            strong_aug_ego_next_obs = torch.cat([strong_aug_ego_next_obs, original_ego_next_obs[:, l - 1:l]], dim=1)

        strong_aug_obs = self.encoder(strong_aug_fix_obs, strong_aug_ego_obs)
        with torch.no_grad():
            strong_aug_next_obs = self.encoder(strong_aug_fix_next_obs, strong_aug_ego_next_obs)

        # update critic
        metrics.update(
            self.update_critic(strong_aug_obs, action, reward, discount, strong_aug_next_obs, step)
        )

        # update actor and alpha
        metrics.update(
            self.update_actor_and_alpha(strong_aug_obs.detach(), step)
        )
